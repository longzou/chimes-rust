...
pub async fn dict_save(req: web::Json<ChimesDictInfo>) -> Result<HttpResponse> {
    ...
    match val.save(rb).await {
        ...
        Err(err) => {
            let ret: web::Json<ApiResult<ChimesDictInfo>> =
...
pub async fn dict_search(req: web::Json<ChimesDictInfo>) -> Result<HttpResponse> {
    ...
    match val.query_list(rb).await {
        ...
        Err(err) => {
            let ret: web::Json<ApiResult<Vec<ChimesDictInfo>>> =
                web::Json(ApiResult::error(5010, &err.to_string()));
            Ok(HttpResponse::Ok().json(ret))
        }
    }
...
pub async fn dict_paged(
    req: web::Json<ChimesDictInfo>,
    ...
    match val.query_paged(rb, current, size).await {
        ...
        Err(err) => {
            let ret: web::Json<ApiResult<Page<ChimesDictInfo>>> =
                web::Json(ApiResult::error(5010, &err.to_string()));
            Ok(HttpResponse::Ok().json(ret))
        }
    }
